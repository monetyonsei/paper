...
rBoltzmann <- function(n, h, J)
{
  d <- length(h)
  S <- MakeS(d)
  M <- 2^d
  p <- dBoltzmann(S, h, J)
  ans <- S[sample(1:M, n, replace = TRUE, prob = p),]
...
MLE.Boltzmann <- function(x, h.init = NULL, J.init = NULL, 
                          eta = 0.5, tol = 1.0e-6, max.iter = 1000)
{
  x.tilde.bar <- colMeans(Make.tilde(x)) 
  ...
  converged <- 0
  ll <- dtheta <- rep(NA, max.iter)
  ll[1] <- mean(log(dBoltzmann(x, h, J)))
  
  for ( iter in 2:max.iter ) {
    #print(iter)
    gr <- x.tilde.bar - Model.meanv(h, J)
    delta <- eta*gr
    theta <- theta + delta  # update
    h <- theta[1:d]
    J <- ivech(theta[-(1:d)], d)
    dtheta[iter] <- max(abs(delta))
    ll[iter] <- mean(log(dBoltzmann(x, h, J)))
    if( abs(ll[iter]/ll[iter - 1] - 1) <= tol ) { 
      converged <- 1
      break
    }
  }
  
  return(list(h = h, J = J, theta = theta,
              log.likelihood = ll, dtheta = dtheta,
...
VEM.Boltzmann <- function(x, N, subj, tol = 1e-6, max.iter = 100, doplot = TRUE, dotrace = TRUE)
{
  # x: stacked data set
  ...
  mu <- matrix(NA, N, D, byrow = TRUE)
  beta <- matrix(NA, nrow = N, ncol = D, byrow = TRUE)
...
### R function for Monte Carlo experiment
...
sim <- function(M, N, d, Tn, doplot = TRUE)
{
  ...
  D <- 0.5*d*(d + 1)
  RMSE.MLE <- RMSE.VEM <- matrix(NA, M, D)
  ...
  for (r in 1:M) {
    ...
    # MLE
    se.mle <- matrix(NA, N, D)
    for ( n in 1:N ) {
      x.n <- x[subj == n,]
      res.mle <- MLE.Boltzmann(x.n)
      se.mle[n,] <- (res.mle$theta - theta[n,])^2
    }
    ...
    # Evaluation
    RMSE.MLE[r,] <- sqrt(colMeans(se.mle))
    RMSE.VEM[r,] <- sqrt(colMeans((theta - res.vem$mu)^2))
  }
  if(doplot) {
    lbub <- c(min(RMSE.VEM, RMSE.MLE), max(RMSE.VEM, RMSE.MLE))
    plot(RMSE.MLE, RMSE.VEM, 
         xlab = "RMSE of MLE", ylab = "RMSE of VEM",
         xlim = lbub, ylim = lbub, pch = 19, col = "navy")
    abline(0, 1, col = 2, lty = 2)
  }
  
  
  return(data.frame(RMSE.MLE = rowMeans(RMSE.MLE), 
                    RMSE.VEM = rowMeans(RMSE.VEM)))
}
