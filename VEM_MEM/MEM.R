...
MLE.Boltzmann <- function(x, h.init = NULL, J.init = NULL, 
                          eta = 0.5, tol = 5.0e-6, max.iter = 1000)
{
...
BMEM.Boltzmann <- function(x, N, subj, tol = 5e-5) 
{
  ...
  # Initialization
  eta <- rep(0, D)
  alpha.h <- alpha.J <- 1/0.15
  alpha <- c(rep(alpha.h, d), rep(alpha.J, D - d))

  # With zero prior & concatenated data
  
  h0 <- eta[1:d]
  J0 <- ivech(eta[-(1:d)], d)
  m.eta <- Model.meanv(h0, J0)
  C.eta <- Model.covM(h0, J0)
  tmp <- colMeans(Make.tilde(x), na.rm = TRUE) - m.eta
  eta <- eta + drop(solve(C.eta + diag(alpha)/nrow(x)) %*% tmp)
  
  # Posterior
  
  mu <- matrix(NA, N, D, byrow = TRUE)  # posterior means
  beta <- matrix(NA, nrow = N, ncol = D, byrow = TRUE)   # posterior precisions
  h0 <- eta[1:d]  # updated hyperparameter
  J0 <- ivech(eta[-(1:d)], d)  # updated hyperparameter
  m.eta <- Model.meanv(h0, J0)  
  C.eta <- Model.covM(h0, J0)
  for ( n in 1:N ) {
    x.n <- x[subj == n,]
    if (sum(is.na(x.n)) > 0) {  # ignore this block
      ...
      Tn <- nrow(x.n)
      tmp <- colMeans(Make.tilde(x.n), na.rm = TRUE) - m.eta
      mu[n,] <- eta + drop(solve(C.eta + diag(alpha)/Tn) %*% tmp)
...
VEM.Boltzmann <- function(x, N, subj, tol = 5e-5, max.iter = 400, doplot = TRUE, dotrace = TRUE)
{
  # x: stacked data set
